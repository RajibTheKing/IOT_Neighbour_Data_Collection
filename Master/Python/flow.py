...
class Flow(object):
  ...
  def __init__(self, graph, flow_id, sub_flow_id, source, destination, period, release_time=0, deadline=None, max_sub_flow_length=10): #ID
    self.graph = graph
    self.flow_number = flow_id
    self.sub_flow_number = sub_flow_id
    self.source = source
  ...
  #   return isinstance(other, Flow) and self.graph == other.graph and self.source == other.destination and self.period == other.period and self.deadline == other.deadline and self.max_sub_flow_length == other.max_sub_flow_length

  def __str__(self):
    return "@flow.py: flow number: {:4d}, sub flow number: {:4d}, source: {:2d}, destination: {:2d}, period: {:4d}, release time: {:4d}, deadline: {:4d}.".format(self.flow_number, self.sub_flow_number, self.source, self.destination, self.period, self.release_time, self.deadline)
  ...
  def output_flow_path(self):
    output = "ID: {:2d} | ".format(self.flow_number)
    for (_, sub_path) in self.path.items():
      for node in sub_path:
        output += "{:2d}; ".format(node)
      output = output[:-2] # default: output = output[:-1] + "\n"
    return output
  ...
  # flag to indicate if a flow miss its deadline (CNMID := Cannot meet its deadline)
  def set_CNMID(self, CNMID):
    self.CNMID = CNMID
